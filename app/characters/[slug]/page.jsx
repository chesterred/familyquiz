...
export async function getCharactersBySlug(slug) {
  const data = await fetch(`${endpoint}/characters/${slug}`)
  return data.json()
}
export default async function Page({ params }) {

  const { character, character_quotes } = await getCharactersBySlug(params.slug);

  return (<Container className='flex flex-col gap-5 py-5' as='main'>
    <div className='flex flex-col gap-2'>
      <h1 className='text-2xl font-semibold capitalize'>{character.name}</h1>
      <ul className='flex gap-1 text-sm'>
        {
          character.occupations.map(item => {
            return (<li key={item} className='p-2 text-gray-300 bg-gray-800 rounded-md'>{item}</li>)
          })
        }
      </ul>
    </div>
    ...
    <ul className='grid gap-2 sm:grid-cols-2'>
      {
        character.images.map(item => {
          return (<li key={item} className='p-2 text-gray-300 bg-gray-800 rounded-md'>
            <Image className='transition-all duration-500 hover:scale-110 hover:rotate-2' src={item} width={760} height={435} />
          </li>)
        })
      }
    </ul>
    {character.skills && (
      <>
      <h2 className='text-xl font-bold'>Power adn Skills</h2>
      <ul className='flex flex-wrap gap-1'>
        {
          character.skills.map(item => {
            return (<li key={item} className='flex justify-center flex-grow px-2 py-1 text-orange-400 rounded-full'>
              {item}
            </li>)
          })
        }
