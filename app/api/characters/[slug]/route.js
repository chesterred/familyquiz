...
export async function GET(req, { params }) {
  try {
    ...
    const character_quotes = qoutes.data.filter(item => item.character_id === character.id)
    return NextResponse.json({ character, character_quotes: character_quotes.length > 0 ? character_quotes : null })
  }
  catch (error) {
