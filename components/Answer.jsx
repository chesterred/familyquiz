...
export const Answer = ({ answers, questionId }) => {
  ...
  useEffect(() => {
    ...
    if(selected) {
      setLoading(true);
      fetch(`/api/quiz/answer/${questionId}`).then((res) => {
        return res.json()
      }).then(data => {
        setLoading(false)
        if(subscribed){
    ...
    <ul className='grid grid-cols-2 gap-2 md:grid-cols-4'>
      {
        answers.map(item => {
          ...
          const isWrong = selected === item && data?.correct !== selected;
          const isCorrect = data?.correct === item;

          return (
            <li key={item} style={{color:"white"}}>
              <button disabled={data || loading} onClick={() => setSelected(item)}
              className={cn('p-2 rounded-md items-center justify-between w-full flex text-sm font-semibold disabled:cursor-not-allowed transition-all' ,
              isLoading && 'animate-pulse' ,
              isWrong ? 'bg-red-700' : 'bg-slate-800' ,
              isCorrect && 'outline text-green-50'
              )}    >
                {item}
                {isCorrect && <FaCheck />}
